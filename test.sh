...
afl-showmap -o /tmp/trace.txt -- julia simple_example.jl < test_corpus/seed1.txt 2>&1 | grep -E "(Captured|fork server)"

# Instructions for full fuzzing
echo ""
echo "4. To run full fuzzing:"
echo "   export AFL_SKIP_BIN_CHECK=1"
echo "   afl-fuzz -i test_corpus -o findings -- julia simple_example.jl"
echo ""
